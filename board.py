...
class Board:
	...
	def draw_control_squares(control_dims, scores, inc, small_font, current_player, p):
		...
		for j in range(6):
			...
			# Arrows for increments in score
			ac = Board.colors[0] if current_player else Board.colors[2]
			aw = inc[p][j] * sz
			ax = sx + sm + scores[p][j] * ss - aw
			pg.draw.rect(Board.screen, ac, (ax, sy + 4 + j * ss + p * py, aw, 4))
	...
	@staticmethod  # FIXME: can now also place free tiles
	def place(current_tile, current_rot, coord=None):
		...
		# print("------ Checking [on_board, is_free] ------")
		# print([Board.is_on_board(coord), Board.is_free_pair(coord, current_rot, free=True)])
		# print([Board.is_on_board(neighbor), Board.is_free_pair(neighbor, current_rot, free=True)])
		#
		# if not coord:
		#
		# 	coord = Board.get_mouse_axial()
		# 	if not Board.is_on_board
		# 	if not Board.is_free(coord, current_rot):
		# 		return False
		# 	if not Board.is_legal((*coord, current_rot)):
		# 		return False
		# if not Board.is_free(coord, current_rot, True) or not Board.is_on_board(coord):
		# 	return False
	...
	@staticmethod  # TODO: move to Hex
	def get_total_hex(n):  # n is the "radius"
		return 3*n*(n-1)+1
